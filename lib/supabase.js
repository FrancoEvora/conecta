import { SUPABASE_KEY, SUPABASE_SERVICE_ROLE_KEY, SUPABASE_URL } from "@/lib/config";

function buildHeaders(accessToken, extra = {}, includeJson = true, apiKey = SUPABASE_KEY) {
  return {
    apikey: apiKey,
...
export async function supabaseRequest(path, options = {}) {
  const { method = "GET", body, accessToken, headers = {}, cache = "no-store", includeJson = true, apiKey = SUPABASE_KEY } = options;
  const response = await fetch(`${SUPABASE_URL}${path}`, {
    method,
    headers: buildHeaders(accessToken, headers, includeJson, apiKey),
...
function assertAdminKey() {
  if (!SUPABASE_SERVICE_ROLE_KEY) {
    const error = new Error("Configuração administrativa do Supabase ausente.");
    error.code = "missing_service_role";
    throw error;
  }
}

export function authAdminCreateConfirmedUser({ email, password, metadata = {} }) {
  assertAdminKey();
...
export function authAdminUpdateUser(userId, payload) {
  assertAdminKey();
...
export function authVerifyOtp({ tokenHash, type }) {
  return supabaseRequest("/auth/v1/verify", { method: "POST", body: { token_hash: tokenHash, type } });
}

export function authResendSignup(email, redirectTo) {
  const query = redirectTo ? `?redirect_to=${encodeURIComponent(redirectTo)}` : "";
  return supabaseRequest(`/auth/v1/resend${query}`, { method: "POST", body: { email, type: "signup" } });
}
