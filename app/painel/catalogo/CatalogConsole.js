...
import { DevelopmentForm, CampaignForm, RewardForm } from "./CatalogEntityForms";
...
export default function CatalogConsole({ context, initialData }) {
  ...
  const campaignsById = useMemo(
    () => Object.fromEntries((data.campaigns || []).map(item => [item.id, item])),
    [data.campaigns]
  );
  ...
  async function reload() {
    setLoading(true);
    try {
      setData(await request("catalog_v2"));
    } catch (error) {
  ...
  async function transition(type, item, action) {
    try {
      ...
      if (action === "restore") {
        await request("catalog_restore_published", {
          p_entity_type: type,
          p_entity_id: item.id,
          p_notes: "Rascunho descartado pelo usuário."
        });
      } else {
        await request("catalog_transition", {
          p_entity_type: type,
          p_entity_id: item.id,
          p_action: action,
          p_notes: notes,
  ...
  async function importCsv(event) {
    ...
    if (!productId || !(file instanceof File) || !file.size) {
      return setToast("Selecione o produto e o arquivo CSV.", true);
    }
  ...
  return <div className={styles.shell}>
    <aside className={styles.sidebar}>
      <NetworkMark inverse/>
      <div className={styles.sideLabel}>Central de produtos</div>
      <nav>{tabs
        .filter(([key]) => key !== "prices" || permissions.price)
        .filter(([key]) => key !== "inventory" || permissions.inventory)
        .map(([key, label, icon]) => <button
          key={key}
          className={active === key ? styles.navActive : ""}
          onClick={() => setActive(key)}
        ><Icon name={icon}/><span>{label}</span></button>)}</nav>
      <div className={styles.sideLinks}>
        <Link href="/painel"><Icon name="arrow"/> Voltar ao painel</Link>
        <Link href="/painel/compartilhamentos"><Icon name="chart"/> Distribuição social</Link>
      </div>
    ...
    <div className={styles.main}>
      <header className={styles.topbar}>
        <div><span className="eyebrow">Central de governança comercial</span><h1>{tabs.find(([key]) => key === active)?.[1]}</h1></div>
        <div className={styles.topActions}>
          <button onClick={reload} disabled={loading}>{loading ? "Atualizando…" : "Atualizar"}</button>
          <span><b>{context.display_name}</b><small>Equipe interna</small></span>
          <button onClick={() => fetch("/api/auth/logout", { method: "POST" }).then(() => location.href = "/")}>Sair</button>
        </div>
      </header>
    ...
    {modal?.type === "development" && <Modal
      title={modal.item ? "Editar estrutura imobiliária" : "Nova estrutura imobiliária"}
      subtitle="Empreendimentos são agrupadores opcionais exclusivos da vertical imobiliária."
      onClose={() => setModal(null)}
      wide
    ><DevelopmentForm item={modal.item} partners={data.partners || []} onClose={() => setModal(null)} onSaved={reload} setToast={setToast}/></Modal>}

    {modal?.type === "product" && <Modal
      title={modal.item?.id ? "Editar produto" : "Novo produto"}
      subtitle="A categoria define o formulário e conecta o produto ao mesmo DNA Comercial usado pelo conector."
      onClose={() => setModal(null)}
      wide
    ><ProductFormV2 item={modal.item || {}} partners={data.partners || []} developments={data.developments || []} onClose={() => setModal(null)} onSaved={reload} setToast={setToast}/></Modal>}

    {modal?.type === "campaign" && <Modal
      title={modal.item ? "Editar campanha" : "Nova campanha"}
      subtitle="Uma campanha define público, período, atribuição e jornada comercial."
      onClose={() => setModal(null)}
      wide
    ><CampaignForm item={modal.item} products={data.products || []} developments={data.developments || []} partners={data.partners || []} onClose={() => setModal(null)} onSaved={reload} setToast={setToast}/></Modal>}
    ...
    {modal?.type === "preflight" && <Modal
      title={modal.result.valid ? "Pronto para aprovação" : "Pendências encontradas"}
      subtitle="Pré-validação editorial e comercial"
      onClose={() => setModal(null)}
    ><div className={modal.result.valid ? styles.preflightGood : styles.preflightBad}><Icon name={modal.result.valid ? "check" : "shield"} size={36}/><h3>{modal.result.valid ? "Todos os critérios obrigatórios foram atendidos." : "Corrija os itens antes de aprovar ou publicar."}</h3>{!modal.result.valid && <ul>{modal.result.errors.map(error => <li key={error}>{error}</li>)}</ul>}</div></Modal>}
  </div>;
}
