...
export default function InviteClient({ invitation, code, socialShareCode = "" }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/evento", {
      ...
      body: JSON.stringify({ code, eventType: "view", sessionId: currentSession, metadata: { path: location.pathname } })
    }).catch(() => {});
    ...
    if (socialShareCode) {
      fetch("/api/social/event", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          publicCode: socialShareCode,
          eventType: "landing",
          sessionId: currentSession,
          metadata: { invitationCode: code, path: location.pathname }
        })
      }).catch(() => {});
    }
  }, [code, socialShareCode]);
  ...
  async function submit(event) {
    ...
    payload.code = code;
    payload.campaignSlug = invitation.campaign_slug;
    ...
    payload.socialShareCode = socialShareCode || null;
    ...
    if (!response.ok) {
      setError(data.error || "Não foi possível registrar sua autorização.");
      setStatus("error");
      return;
    }
    ...
    fetch("/api/evento", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ code, eventType: "form_submitted", sessionId: sessionId(), metadata: { alternative } })
    }).catch(() => {});
  }
